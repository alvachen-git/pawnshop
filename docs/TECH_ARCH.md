...
默认入口为 `scenes/start.tscn` → `aqi_manifest.json`，内容与存档载荷均为22。`RunSession`协调经营、准备、铜镜、熟客、行情、夜客风险及寝屋观察／阿七收尾；`SaveLibrary`管理分运行自动位、六个手动槽及失败记录，读取按原运行与内容恢复。职业赎回策略虽在代码中存在，默认v22继续未启用，v20独立内容保留。房间已接入美术和独立镜组件，异象预览不等于正式剧情。当前规则、完整日阶段、存档行为与已知缺口以 [当前开发规格](CURRENT_SPEC.md) 为准。

以下架构章节保留历史演进信息，包括旧版本号、检查点列表和阶段流，不作为当前API契约；维护现有功能时须核对对应代码与当前规格。
...
本段及后文版本号、检查点和玩法描述均为当时的历史架构记录；21/49夜本金系统仍未接入，七夜内容已在后续版本实现。
