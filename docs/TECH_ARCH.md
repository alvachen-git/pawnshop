...
M0–M2已由项目负责人确认。M3增加买家出售、库存权属、活当/赎回/续当接口/绝当及财务账本。当前仍无EventDirector、完整内容编排或鬼货风险。
...
当前支持 `items`、`customers`、`runs`、`buyers`、`pawn_terms`。Manifest的default_run_id决定启动配置，RunDefinition包含夜数、开铺分钟、夜间时长、时间步长、初始现金、seed与占位行动。RunDTO只存在于映射边界；RunDefinition复制标量并生成DayActionDefinition，不持有源Dictionary/DTO。定义使用只读getter，集合返回副本。GDScript的下划线字段是内部约定，不是语言级私有隔离。
...
- 当前save_version=3、content_version=4。版本变化会拒绝旧存档，不进行静默迁移；未来确有兼容用例时再实现迁移器。
...
- RunDefinition.customer_slots定义arrival/customer_id/item_id/variant_id；item_id或variant_id为空时，从内容池按固定seed抽取。每夜预生成一次，不因查询UI/重复鉴定重新抽取。当前是测试客流，不代替M4的EventDirector。
...
- 顾客口供属于未证实信息，正式询问只做M2最小问答，不实现M4的通用对白图或事件效果。
...
- NightResolutionView + NightResolutionPresenter：夜间结算占位、日结与本轮结束展示。
