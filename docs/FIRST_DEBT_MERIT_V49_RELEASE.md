...
基线为 main 银楼v48与青帮提交 `4c239f7`，在隔离分支 `codex/first-debt-merit-release-v48` 整合。默认运行 `first_debt_merit_release`，独立自动位与档案库遵循v47的版本隔离方式。原开发树 `.artifacts/first-debt-merit` 保留。
...
v49复制银楼v48运行配置，只增加新运行ID和香炉事件；保留银楼解锁、银价与收货，以及开铺前回收、陆掌眼卖货与教学、利息选择、相机及留声机鉴定。青帮、特殊客与周怀安独立入口维持主线现状，没有擅自并入默认局。
