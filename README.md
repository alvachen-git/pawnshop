...
**当前本地统一试玩：v29 品相与掌眼。** 双击 `play-unified.cmd` 开始正常十夜；`-Stage no-bench` 试玩无鉴物台的破损检查，`-Stage minor` / `-Stage major` 试玩轻损／重损，`-Stage stack` 比较两种压价，`-Stage fan` 体验案上对证。保留设施、铜镜、阿七、命灯和寝屋。见[试玩说明](docs/FAN_CONDITION_PLAYTEST.md)、[接入安排](docs/UNIFIED_PLAYTEST_INTEGRATION.md)、[验收](docs/FAN_CONDITION_ACCEPTANCE.md)。`play-v28.cmd`、`play-v27.cmd` 保留旧版启动与存档。以下旧版本说明供历史查阅。

新增独立的 **当铺成长 v25**：一级鉴物台、陈列柜与买家、第十七柜探索、完整设施场景，以及小箭头和默认淡入淡出。双击 `启动当铺成长.cmd`，或查看[启动与试玩说明](docs/SHOP_GROWTH_PLAYTEST.md)、[合并验证](docs/SHOP_GROWTH_INTEGRATION.md)。旧版本入口与已有存档保留。

旧独立入口 **v26 镜中重逢**：保留十夜经营、阿七、命灯与寝屋，铜镜正式对质改为双人逐句对白、固定概率交涉和四种收尾。丈夫在妻儿去世前另组家庭；逼问或打圆场决定其反应，激怒后可劝解或让女子杀夫。能力变化及铜镜怨气只在结局完成后通知。可暂缓或完全不处理，第十夜仍结束试玩。详见 [v26完整启动与试玩说明](docs/MIRROR_REUNION_V26.md)。
