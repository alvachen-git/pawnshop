...
固定柜台式 2D 当铺经营与规则恐怖游戏。当前整合版使用v11内容：默认 `p0_variety` 保留三夜原型，新增独立 `ordinary_four` 四夜普通经营样板。支持八类随机人物、专属鉴定、来源核验、证据议价、一次性贬低、原主回访与寝屋流程。尚未制作七夜 Demo 或第21/49夜还本系统。
...
当前有8种普通物品、8类顾客。三夜原型保留铜镜、原买家和短当约；四夜样板新增有限收货预约与三夜当约，不调度鬼货或主线事件。正式12图需求保留在 [美术交接](docs/M7_ART_HANDOFF.md)。完整鬼市、七笔阴账、49夜剧情、准备行动和行情链仍属后续范围。

## 存档

新局使用 **save_version=11 / content_version=11**。三夜原型保存于 `user://p0/autosave_v11.json`，四夜样板保存于 `user://ordinary_four/autosave_v11.json`。三夜入口可导入通过完整历史校验的旧v9/v10局，原文件保留；旧局继续使用对应旧内容，重新开局才进入v11。旧v10分别保留实施前测试快照和已上线版本的配置，包含两种历史现金/债务组合。

- 源码/编辑器：`%APPDATA%/Godot/app_userdata/鬼市当铺/p0/autosave_v11.json`，四夜样板使用相邻的 `ordinary_four/autosave_v11.json`
