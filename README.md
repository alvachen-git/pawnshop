...
默认新游戏为 **v23 十夜旧事**：整合铜镜辨生死、两类阴客、一次报价客、湿包客、命灯、寝屋照片与私人信件，完整经营至第十夜。铜镜支线新增30银元付费查访和丈夫再次会面，按线索与实际行动推进；8→9→10只是示例路线，可以延后或不调查。详见 [v23试玩与验证](docs/INVESTIGATION_V23.md)。
...
默认十夜新局使用 **save_version=23 / content_version=23**，自动位置为 `auto/mirror_investigation_ten`，位于既有档案库 `user://save_library/library_v1.json`；独立路径为 `user://mirror_investigation_ten/autosave_v23.json`。命灯受害仍自动保存，营业中不能手动随时保存。旧v22铜镜版、命灯版及更早存档按原内容读取，不迁移、不追溯补扣伤害。
...
- 鬼市当铺_GDD_V0.9.docx：当前设计输入，以任务中最终确认的计划解决版本冲突。
