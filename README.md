...
**珍珠试玩入口：`play-pearl-v38.cmd -Stage pearl -Wide`，正式新局可用 `play-unified.cmd` 或 Godot 项目入口。** 默认版本为 v40 `bangle_unified`，加入第一账、龙镯追查与第十八夜后继续营业，并保留v38的珍珠逐粒查验、整串谈价，以及五位富客按权重携带十种高档货。实际货值在生成时固定，手记、对客说法与客人认识各自保存。详见 [v38试玩与验收](docs/PEARL_V38_LOCAL.md)。
...
当前规格与待确认分歧统一见 [当前开发规格](docs/CURRENT_SPEC.md)。默认v40保留线上v23已整合的职业赎回、生计背景与寝屋铜镜反馈；原v21夜客和阿七七夜版使用各自冻结规则。第七夜阿七收尾后不额外叠加墙镜异象。

新增商品与行家复核版作为独立七夜入口保留，包含银戒指、银锁、折扇、茶盏及验配出售；启动方式见 [商品与行家说明](docs/GOODS_EXPERTISE.md)。库存详情统一精简为已知品相要点。

旧版本内容与存档保留原规则，v19经营整合、v20职业赎回和v21商品复核成果继续包含在新局中。换物不改变原票本金、赎金、期限或原主赎回概率；两类阴客与通用辨生死在对应v22、v23及新默认v40中生效。
...
默认新局使用 **save_version=39 / content_version=39**，自动位置为 `auto/bangle_unified`，位于既有档案库 `user://save_library/library_v1.json`；独立路径为 `user://bangle_unified/autosave_v40.json`。旧v31 `wealthy_ten`、v30 `unified_ten` 和更早存档继续按各自规则运行，不补抽破损、不追收器材、不改写原当票。命灯受害仍自动保存，营业中不能手动随时保存。
...
整合第一账、龙镯追查、陈小满预约与归还收尾，同时保留v38富客、钟表与珍珠鉴定。默认新游戏使用v40；历史运行按原manifest恢复。第一账开发详情见`docs/FIRST_DEBT_V31.md`，整合验证见`docs/qa/first-debt-release/REPORT.md`。

金镯发布整合版：默认入口已更新为 v40，包含已发布的首笔旧债剧情及老凤祥花雕金镯称重、火试、谈价。`play-unified.cmd -Stage bangle` 可隔离试玩；v39 两条存档线保留，详见 [金镯说明](docs/BANGLE_V39_LOCAL.md)。
