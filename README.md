...
默认新游戏使用 **v15七夜开铺准备**：第一夜接开场剧情，从第二夜起每夜可准备两次。支持招揽客人、定向收货、备茶候客和打听来客；按钮保留名称、费用与次数，效果通过鼠标悬停查看。旧进度继续原规则，详见 [开铺准备说明](docs/OPENING_PREPARATION.md)。
...
默认七夜新局使用 **save_version=15 / content_version=15**，自动位置为 `auto/prepared_seven`，位于既有档案库 `user://save_library/library_v1.json`；独立路径为 `user://prepared_seven/autosave_v15.json`。版本14和更早存档按原内容还原。
