...
固定柜台式 2D 当铺经营与规则恐怖游戏。当前新内容为 **v10 三夜普通交易**，默认 `p0_variety`：8件普通物品各3种品相、8类随机当户、独立姓名与来源核验。保留每日息费、铜镜、寝屋、原主优先赎回及夜末留货或转当。旧局使用独立的 v9 内容继续。当前尚未制作七夜 Demo 或第21/49夜还本系统。
...
当前有8种普通物品、1件铜镜、8类顾客、4名买家、2种当约、7个普通事件和1段铜镜遭遇。新增四类使用独立SVG低保真立绘。正式12图需求保留在 [美术交接](docs/M7_ART_HANDOFF.md)。完整鬼市、七笔阴账、49夜剧情、准备行动和行情链仍属后续范围。

## 存档

当前 **save_version=10 / content_version=10**，默认 `user://p0/autosave_v10.json`。新档不存在时，优先导入旧 v9 房间档；无 v9 时可读取 v8 房间档。旧局继续使用 `data/legacy/content_v9.json` 的原内容与账目，后续写入新文件，保留旧文件；重新开始新游戏才进入 v10 内容。v7/v8兼容校验仍保留。

- 源码/编辑器：`%APPDATA%/Godot/app_userdata/鬼市当铺/p0/autosave_v10.json`
