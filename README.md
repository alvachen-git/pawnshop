...
默认新游戏使用 **v22《接铺》收尾版**：沿用 v21 经营与风险，第二／第五夜起可在寝屋主动发现两处生活痕迹；第七夜核账并处理铺内危机后，阿七像普通客人一样在原柜台前登场，点击她交谈、修纸风车，再翻到“尚欠七账”与可选旧铺草图。具体分支、兼容与本地测试见 [阿七与七夜收尾](docs/AQI_SEVEN.md)。此前夜客内容见 [深夜经营说明](docs/NIGHT_MARKET.md)。

当前规格与待确认分歧统一见 [当前开发规格](docs/CURRENT_SPEC.md)。v20 职业赎回概率与生计背景通过独立内容、旧档读取和专项测试保留，**默认v22继续不启用职业赎回开关**，见 [职业赎回说明](docs/PAWN_REDEMPTION_CHANCE.md)。寝屋美术及独立镜组件已接入，正式剧情尚未触发雾化、扭曲等预览异象。
...
普通交易采用 0.72 秒自动交割反馈，结束后继续接待；首单手动盖章保留。最近结果可收起，详细凭据可从该条目或账本主动复查。当前节奏规格与玩家验收步骤见 [交易反馈补充](docs/TRADE_FEEDBACK_POLISH.md)。
...
夜末流程：**封铺→合账（当票到期、息费）→铺内应对→第七夜阿七与阴账收尾→回房→床确认就寝→个人应对→天明日结**。回房后不能经营。无待办且未到末夜时，「放松入眠」完成后直接进入次日，末夜保留摘要。寝屋已采用紧凑衣柜版美术，命灯、旧信、照片、床和日常观察已接入；墙镜使用独立组件，正式游戏只显示正常倒影及命灯联动。见 [寝屋美术](docs/BEDROOM_ART.md)与[镜面边界](docs/BEDROOM_MIRROR.md)。
...
默认七夜新局使用 **save_version=21 / content_version=21**，自动位置为 `auto/night_market`，位于既有档案库 `user://save_library/library_v1.json`；独立路径登记为 `user://night_market/autosave_v21.json`。版本20及更早存档按对应内容还原，不重抽既有当票，也不向旧局插入新规则；读取旧存档再返回标题，新游戏仍进入v21。旧v16自动档及红布操作历史保留。

六个手动槽支持读取较早可玩进度，建立新的尝试标识并保留失败记录；营业中不能手动保存。当前行为与GDD绝当不回档目标的分歧见 [规格差异](docs/CURRENT_SPEC.md#5-规格差异与处理状态)。源码工作树共享项目用户目录，不天然隔离存档。
...
- [当前开发规格](docs/CURRENT_SPEC.md)：默认v21的规则、实现边界、证据与待确认分歧。
