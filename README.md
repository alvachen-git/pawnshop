...
默认新游戏使用 **v16泣血铜镜第一章**：从开场连续经营七夜，保留开铺准备、赎当及钢笔预约，并加入窥见交易隐情、调查旧当票与顾先生隐瞒的剧情。启动、能力规则、存档兼容与试玩步骤见 [铜镜第一章说明](docs/MIRROR_CHAPTER.md)。
...
默认七夜新局使用 **save_version=16 / content_version=16**，自动位置为 `auto/mirror_chapter`，位于既有档案库 `user://save_library/library_v1.json`；独立路径为 `user://mirror_chapter/autosave_v16.json`。版本15、14和更早存档按原内容还原；读取旧存档再返回标题，新游戏仍进入新版。已有v16记录保留原红布耗时历史，之后盖好或揭开红布均免费。
