...
static func finish(data: Dictionary, state: RunState, run: RunDefinition) -> String:
	...
	var failed := not SaveTimeline.unsettled(state) and state.phase != &"pre_open" and FeeService.overdue(state) > 0 and state.risk_pending.is_empty() and state.phase != &"dead" and String(state.phase) not in RoomFlow.PHASES
