...
func restore(data: Variant, run: RunDefinition, catalog: ContentCatalog, extended: bool) -> RunState:
	...
	var version := SocialRules.VERSION if SocialRules.enabled(run) else ShopGrowthService.VERSION if ShopGrowthService.enabled(run) else 23
	...
	var legacy_intro: bool = version == 27 and data.get("social") is Dictionary and not data.social.has("intro_step")
	...
	for index in range(start, data.action_journal.size()):
		...
		if row.method in ["growth_command", "social_command"] and not result.ok: return null
	...
	if version == 26: expected = SocialCopyMigration.normalize(expected)
	var actual := session.read_state()
	if version == 27: SocialCopyMigration.v27_notices(expected, actual)
