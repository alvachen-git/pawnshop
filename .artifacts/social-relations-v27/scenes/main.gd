...
func _ready() -> void:
	if OS.is_debug_build():
		for argument in OS.get_cmdline_user_args():
			if argument.begins_with("--social-preview=") and argument.trim_prefix("--social-preview=") in ["introduction", "preparation", "closure", "supply", "claim", "respected", "disliked", "delivery", "near_plaque", "plaque", "intimidation", "cotton"]:
				_bootstrap.social_preview = argument.trim_prefix("--social-preview=")
	...
	if not _bootstrap.social_preview.is_empty() and _bootstrap.session != null: print("SOCIAL V27 PREVIEW: " + _bootstrap.social_preview)
	...
	if not _bootstrap.social_preview.is_empty() and _bootstrap.session != null and _bootstrap.social_preview != "introduction":
		_counter_screen.get_node("%ScreenFlowCoordinator").show_panel.call_deferred(&"social")
