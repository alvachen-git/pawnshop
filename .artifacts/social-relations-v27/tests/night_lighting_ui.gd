...
func _run() -> void:
	...
	# Compare the actual rendered light contribution in the darkest band.
	stage._paint_material.set_shader_parameter("incense_strength", 0.0)
	await _frames()
	var unlit := root.get_texture().get_image()
	stage._paint_material.set_shader_parameter("incense_strength", 1.0)
	await _frames()
	var lit := root.get_texture().get_image()
	_check(brightness(lit, Rect2(.035,.66,.09,.19)) > brightness(unlit, Rect2(.035,.66,.09,.19)) * 1.3, "embers softly illuminate the nearby burner and wood")
	_check(is_equal_approx(brightness(lit, Rect2(.65,.025,.035,.09)), brightness(unlit, Rect2(.65,.025,.035,.09))), "incense glow does not illuminate the distant street")
	await _capture("incense")
