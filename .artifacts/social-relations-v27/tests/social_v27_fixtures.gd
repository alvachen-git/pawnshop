...
func run() -> void:
	...
	save_fixture(second_night(), "introduction")
	...
	save_fixture(delivery_session(), "delivery")
	for stage in ["near_plaque", "plaque"]:
		run_def = catalog.get_definition("runs","social_preview_" + stage)
		var prepared := second_night()
		if prepared._day.state.social.pending.get("kind","") == "supply": prepared.social_command("decline_supply")
		save_fixture(prepared, stage)
		if stage == "plaque":
			active_ordinary(prepared)
			save_fixture(prepared,"intimidation")
