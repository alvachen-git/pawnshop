...
func _run() -> void:
	...
	_capture_prefix = "complete_early_1600" if "wide" in OS.get_cmdline_user_args() else "complete_early_1280"
	...
	for decision in ["early_redeem", "defer_redeem"]:
		...
		_main.get_node("Bootstrap").save_path = "res://.godot/qa/complete_early/ui_auto.json"
		root.add_child(_main)
		_session = _main.get_node("Bootstrap").session
		_session._save.library.path = "user://tests/complete_early_ui/%d.json" % Time.get_ticks_usec()
		...
		var data: Dictionary = JSON.parse_string(FileAccess.get_file_as_string("res://.godot/qa/complete_early/allow_%d_pre_open.json" % story.follow_night))
		var codec := SaveCodec.new()
		var state := codec.decode(data, _session.definition, 19, driver.catalog, true)
		_check(state != null, "real pre-return fixture")
		var lib := _session._save.library
		_check(lib.write_entry("manual/1", state, _session.definition, 19, driver.catalog), "manual save")
	...
	print("COMPLETE EARLY UI: %d assertions, %d failures" % [_assertions, _failures])
