...
func progress(s: RunSession) -> bool:
	...
	var steps: Array = [["fd_ticket", "read"], ["fd_lu", "ask"], ["fd_chen_request", "ask"], ["fd_family", "read"]]
	...
	for pair in steps:
		...
		if route.begins_with("natural"): fixture(s, "before-" + str(pair[0]))
		fd(s, pair[0], pair[1])
		if route.begins_with("natural"): fixture(s, "after-" + str(pair[0]))
		return true
	return false
...
func earn(s: RunSession, v: CustomerVisit) -> bool:
	...
	if item.category != MarketService.category(s._day): return false
...
func fixture(s: RunSession, stage: String) -> void:
	DirAccess.make_dir_recursive_absolute(ProjectSettings.globalize_path("res://.godot/qa/v28/"))
	var file := FileAccess.open("res://.godot/qa/v28/" + ("pay-" if route == "natural_pay" else "late25-" if route == "natural_late25" else "") + stage + ".json", FileAccess.WRITE)
