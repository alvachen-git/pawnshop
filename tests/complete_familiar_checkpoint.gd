...
func run() -> void:
	...
	definition = catalog.get_definition("runs", "complete_seven")
	var directory := "res://.godot/qa/complete_familiar/cross"
	...
	for name in files:
		...
		var restored := codec.decode(data, definition, 19, catalog, true)
		...
		check(codec.decode(bad, definition, 19, catalog, true) == null, "funds cannot change on restore")
		...
		if not bad.scenario_history.is_empty():
			...
			check(codec.decode(bad, definition, 19, catalog, true) == null, "duplicate action refused")
...
func save_failures(state: RunState) -> void:
	var library := SaveLibrary.new("res://.godot/qa/complete_familiar/library_failure_%d.json" % Time.get_ticks_usec())
	check(library.write_entry("manual/1", state, definition, 19, catalog), "manual slot valid before failed replacement")
	var bytes := FileAccess.get_file_as_bytes(library.path)
	var snapshot := state.to_read_model()
	library.fail_write = true
	check(not library.write_entry("manual/1", state, definition, 19, catalog), "write failure reported")
	check(FileAccess.get_file_as_bytes(library.path) == bytes and state.to_read_model() == snapshot, "old slot and state retained")
	check(not library.write_entry("manual/2", state, definition, 19, catalog), "failed new slot not published")
	library.fail_write = false
	check(library.write_entry("manual/1", state, definition, 19, catalog), "retry slot succeeds")
	var saved := library.read_entry("manual/1")
	check(not saved.is_empty(), "cross entry catalog resolves v19")
	var manager := SaveManager.new("res://.godot/qa/complete_familiar/unused.json")
...
func completed_routes() -> void:
	for route in ["normal", "candid", "guarded", "elsewhere", "insult_reject", "exhaust", "timeout"]:
		var data: Dictionary = JSON.parse_string(FileAccess.get_file_as_string("res://.godot/qa/complete_familiar/%s_507_final.json" % route))
	...
	for route in ["funded", "short", "unsold", "transfer", "no_pawn", "independent"]:
		...
		var data: Dictionary = JSON.parse_string(FileAccess.get_file_as_string("res://.godot/qa/complete_familiar/%s_final.json" % filename))
