...
func load_stage(stage: String) -> RunSession:
	var raw = JSON.parse_string(FileAccess.get_file_as_string("res://.godot/qa/v29/" + stage + ".json"))
	var codec := SaveCodec.new()
	var st := codec.decode(raw, run_def, 29, catalog, true)
	check(st != null, "load " + stage + " " + codec.error_message)
	if st == null: return null
	var store := GhostReplayStore.new(); store.origin = raw.ghost_origin
	var s := RunSession.new(run_def, 29, store, catalog); s._day.state = st
	return s
...
func run() -> void:
	...
	for stage in ["before-fd_ticket", "before-fd_lu", "before-fd_chen_request", "before-fd_family", "before-fd_dragon", "before-fd_truth", "before-fd_settle", "before-fd_customer_ticket", "pay-before-fd_compensation"]:
		var s := load_stage(stage)
		if s == null: quit(1); return
		var id: String = stage.trim_prefix("pay-").trim_prefix("before-")
		var choice: String = {"fd_ticket":"read", "fd_lu":"ask", "fd_chen_request":"ask", "fd_family":"read", "fd_dragon":"buy", "fd_truth":"tell", "fd_settle":"return", "fd_customer_ticket":"read", "fd_compensation":"offer"}[id]
		var lib := SaveLibrary.new("res://.godot/qa/v29/durable-" + id + ".json")
		...
		var f := FileAccess.open("res://.godot/qa/v29/committed-" + id + ".json", FileAccess.WRITE)
	...
	for amount in [299, 300]:
		s = load_stage("pay-before-fd_settle"); s._day.state.cash = amount
	...
	var clean := SaveCodec.new().encode(s._day.state, 29)
	for flag in ["fd_family_read", "fd_clear", "fd_truth_told", "fd_dragon_bought", "fd_customer_ticket_read", "fd_protection_read", "fd_yin_link_read", "fd_yin_echo_seen", "fd_compensation_agreed"]:
		var fake := clean.duplicate(true); fake.narrative_flags.append(flag)
		check(SaveCodec.new().decode(fake, run_def, 29, catalog, true) == null, "forged flag " + flag)
	var bad := clean.duplicate(true); bad.cash += 300
	check(SaveCodec.new().decode(bad, run_def, 29, catalog, true) == null, "forged money")
	bad = clean.duplicate(true); bad.action_journal.append({"method":"event_command", "args":["fd_settle", "pay"]})
	check(SaveCodec.new().decode(bad, run_def, 29, catalog, true) == null, "invalid action rejected")
...
func durable_action(scenario: String) -> void:
	...
	var lib := SaveLibrary.new("res://.godot/qa/v29/durable-action-" + scenario + ".json")
	var store := SaveManager.new(); store.library = lib; store.catalog = catalog; s._save = store
	check(lib.write_entry("auto/first_debt_reckoning", s._day.state, run_def, 29, catalog), "initial " + scenario)
	var before := s.read_state(); var bytes := FileAccess.get_file_as_bytes(lib.path)
	lib.fail_write = true
	...
	var file := FileAccess.open("res://.godot/qa/v29/committed-action-" + scenario + ".json", FileAccess.WRITE)
