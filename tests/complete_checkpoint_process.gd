...
func run() -> void:
	var catalog := JsonContentProvider.new("res://data/complete_seven_manifest.json").load_catalog().catalog
	var definition := catalog.get_definition("runs", "complete_seven") as RunDefinition
	var files := DirAccess.get_files_at("res://.godot/qa/complete_early")
	check(files.size() >= 100, "actual route checkpoints")
	for name in files:
		if not name.ends_with(".json"): continue
		var data: Dictionary = JSON.parse_string(FileAccess.get_file_as_string("res://.godot/qa/complete_early/" + name))
		var codec := SaveCodec.new()
		var state := codec.decode(data, definition, 19, catalog, true)
		...
		if not data.pawn_tickets.is_empty():
			...
			check(codec.decode(bad, definition, 19, catalog, true) == null, "cannot alter redemption amount")
		for record in data.scenario_history:
			if record.command == "early_redeem":
				var bad := data.duplicate(true)
				bad.scenario_history.append(record.duplicate(true))
				check(codec.decode(bad, definition, 19, catalog, true) == null, "duplicate early action rejected")
				bad = data.duplicate(true)
				for malformed in bad.scenario_history:
					if malformed.command == "early_redeem": malformed.erase("minute")
				check(codec.decode(bad, definition, 19, catalog, true) == null, "malformed early time rejected without crashing")
				break
		if state.phase != &"private_room": continue
		var library := SaveLibrary.new("user://tests/complete_checkpoint/%s" % name)
		check(library.write_entry("manual/1", state, definition, 19, catalog), "save v18 to shared slot")
		var bytes := FileAccess.get_file_as_bytes(library.path)
		var before := state.to_read_model()
		library.fail_write = true
		check(not library.write_entry("manual/1", state, definition, 19, catalog), "failed overwrite")
		...
		check(library.write_entry("manual/1", state, definition, 19, catalog), "retry save")
		var loaded := library.read_entry("manual/1")
		check(not loaded.is_empty() and loaded.catalog.content_version == 19, "load matches new content")
	print("COMPLETE CHECKPOINTS: %d checks, %d failures" % [checks, failures])
	quit(0 if failures == 0 else 1)

func night_rollback(state: RunState, definition: RunDefinition, catalog: ContentCatalog) -> void:
	var library := SaveLibrary.new("user://tests/complete_checkpoint/rollback_%d.json" % Time.get_ticks_usec())
	check(library.write_entry("manual/1", state, definition, 19, catalog), "save after early redemption")
	var manager := SaveManager.new("user://tests/complete_checkpoint/unused.json")
	manager.library = library; manager.catalog = catalog
	var session := RunSession.new(definition, 19, manager, catalog)
