...
func run() -> void:
	var catalog := JsonContentProvider.new("res://data/familiar_early_manifest.json").load_catalog().catalog
	var definition := catalog.get_definition("runs", "familiar_early") as RunDefinition
	var files := DirAccess.get_files_at("res://.godot/qa/early")
	check(files.size() >= 100, "actual route checkpoints")
	for name in files:
		if not name.ends_with(".json"): continue
		var data: Dictionary = JSON.parse_string(FileAccess.get_file_as_string("res://.godot/qa/early/" + name))
		var codec := SaveCodec.new()
		var state := codec.decode(data, definition, 18, catalog, true)
		...
		if not data.pawn_tickets.is_empty():
			...
			check(codec.decode(bad, definition, 18, catalog, true) == null, "cannot alter redemption amount")
		for record in data.scenario_history:
			if record.command == "early_redeem":
				var bad := data.duplicate(true)
				bad.scenario_history.append(record.duplicate(true))
				check(codec.decode(bad, definition, 18, catalog, true) == null, "duplicate early action rejected")
				bad = data.duplicate(true)
				for malformed in bad.scenario_history:
					if malformed.command == "early_redeem": malformed.erase("minute")
				check(codec.decode(bad, definition, 18, catalog, true) == null, "malformed early time rejected without crashing")
				break
		if state.phase != &"private_room": continue
		var library := SaveLibrary.new("user://tests/early_checkpoint/%s" % name)
		check(library.write_entry("manual/1", state, definition, 18, catalog), "save v18 to shared slot")
		var bytes := FileAccess.get_file_as_bytes(library.path)
		var before := state.to_read_model()
		library.fail_write = true
		check(not library.write_entry("manual/1", state, definition, 18, catalog), "failed overwrite")
		...
		check(library.write_entry("manual/1", state, definition, 18, catalog), "retry save")
		var loaded := library.read_entry("manual/1")
		check(not loaded.is_empty() and loaded.catalog.content_version == 18, "load matches new content")
...
func night_rollback(state: RunState, definition: RunDefinition, catalog: ContentCatalog) -> void:
	...
	check(library.write_entry("manual/1", state, definition, 18, catalog), "save after early redemption")
	var manager := SaveManager.new("user://tests/early_checkpoint/unused.json")
	manager.library = library; manager.catalog = catalog
	var session := RunSession.new(definition, 18, manager, catalog)
