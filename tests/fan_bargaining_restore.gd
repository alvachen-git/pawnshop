...
func _initialize() -> void:
	var catalog:=JsonContentProvider.new("res://data/fan_bargaining_manifest.json").load_catalog().catalog
	var run:=catalog.get_definition("runs",catalog.default_run_id) as RunDefinition
	var count:=0
	for stage in ["ordinary-ready","ordinary-pressed","informed-pressed","urgent-bought","saved-sale"]:
		var payload: Dictionary=JSON.parse_string(FileAccess.get_file_as_string("res://.godot/qa/fan-bargaining/"+stage+".json"))
		var codec:=SaveCodec.new()
		var state:=codec.decode(payload,run,28,catalog,true)
		...
		if codec.decode(payload,run,28,catalog,true)==null: push_error("fork replay " + codec.error_message); quit(1); return
		count+=3
	print("FAN BARGAINING COLD RESTORE: %d checks passed" % count)
	quit(0)
