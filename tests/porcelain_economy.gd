...
func run() -> void:
	...
	for channel in ["single","batch"]:
		var s:=porcelain("yuan","fine");var v:CustomerVisit=s._day.state.visits[0]
		var o:=PorcelainEconomy.owner(s._day.state,v);o.belief={"era":"republic","craft":"rough"};o.rate=60
		PorcelainEconomy.reprice_initial(s._day.state,v)
	...
	for redeem in [false,true]:
		var s:=porcelain("qing","fine","minor",false,"customer_wealthy_antique","pawn");var v:CustomerVisit=s._day.state.visits[0]
		...
		if redeem:
			...
			check(offer==roundi(fixed.porcelain_value.actual*.9),"display uses actual value after default")
	...
	var poor:=porcelain();var visitor:CustomerVisit=poor._day.state.visits[0];poor._day.state.cash=0;var snapshot:=poor.read_state()
	...
	print("PORCELAIN ECONOMY: %d passes, %d failures" % [passes,failures]);quit(0 if failures==0 else 1)
