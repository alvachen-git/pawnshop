...
func verify(s: RunSession, label: String) -> void:
	...
	check(restored != null,"v34 cold replay "+label+" "+codec.error_message)
	if restored != null: check(GhostSaveCodec.same(restored.to_read_model(),s.read_state()),"exact v34 replay "+label)
...
func seal_stage(s: RunSession, v: CustomerVisit, tier: int, wrong := false) -> void:
	...
	if behavior != "stable":
		check(command(s,id,{"op":"mark","clip":"wound/vertical" if behavior == "positional" else "wound/flat","time":2.5+float(v.item.goods.watch_value.offset) if behavior == "positional" else 6.0}).ok,"audible anomaly mark")
	...
	check(s.counter_command("watch_bluff",v.visit_id).ok,"natural bluff transaction")
	verify(s,"natural bluff cold replay")
