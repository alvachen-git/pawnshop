...
var process_manifest := "res://data/aqi_companion_manifest.json"
var process_dir := "res://.godot/qa/v25/"

func run() -> void:
	var loaded := JsonContentProvider.new(process_manifest).load_catalog()
	catalog = loaded.catalog
	run_def = catalog.get_definition("runs", catalog.default_run_id)
	var codec := SaveCodec.new()
	var total := 0
	for filename in DirAccess.get_files_at(process_dir):
		if not filename.ends_with(".json") or filename.begins_with("durable-") or filename.begins_with("ui-") or filename.begins_with("pointer-library-"): continue
		var raw = JSON.parse_string(FileAccess.get_file_as_string(process_dir + filename))
		...
		if not state.pending_event_id.is_empty():
			...
			check(codec.decode(codec.encode(s._day.state, catalog.content_version), run_def, catalog.content_version, catalog, true) != null, "next segment replays")
		var forged: Dictionary = raw.duplicate(true); forged.content_version = 23
		check(codec.decode(forged, run_def, catalog.content_version, catalog, true) == null, "version tamper rejected")
