...
func run() -> void:
	...
	check(loaded.is_success(), "v24 catalog")
	...
	for route in (["example"] if "fixtures-only" in OS.get_cmdline_user_args() else ["early", "example", "partial", "late", "ignore", "sold"]):
		journey(route)
		if failures > 0: break
	print("AQI INVESTIGATION: %d passes, %d failures" % [passes, failures])
...
func fresh23() -> RunSession:
	...
	return RunSession.new(run_def, 24, store, catalog)
...
func verify(s: RunSession, label: String) -> void:
	...
	var data := codec.encode(s._day.state, 24)
	var restored := codec.decode(data, run_def, 24, catalog, true)
	...
	check(codec.decode(forged, run_def, 24, catalog, true) == null, "cash forgery " + label)
...
func fixture(s: RunSession, stage: String) -> void:
	DirAccess.make_dir_recursive_absolute(ProjectSettings.globalize_path("res://.godot/qa/v24"))
	var file := FileAccess.open("res://.godot/qa/v24/" + stage + ".json", FileAccess.WRITE)
	file.store_string(JSON.stringify(SaveCodec.new().encode(s._day.state, 24)))
...
func aqi_story(s: RunSession, route: String) -> void:
	...
	var codec := SaveCodec.new()
	var forged := codec.encode(s._day.state, 24)
	forged.narrative_flags.append("aq_forged")
	check(codec.decode(forged, run_def, 24, catalog, true) == null, "reject unjournaled flag")
