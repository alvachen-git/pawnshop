...
func run() -> void:
	...
	for which in ["return", "pay", "sell_pay", "defer", "miss", "sold_search", "late25", "natural"]:
		...
		run_def._initial_cash = 300 if route == "natural" else 2000
		search_night = 0; finish_night = 0
		journey()
		if failures: break
	print("DRAGON ROUTES: %d passes, %d failures" % [passes, failures])
...
func journey() -> void:
		...
		for step in 180:
			...
			if v != null:
				...
				if v.customer_id == "fd_seller" and n >= begin:
					...
					check(s.counter_command("offer", v.visit_id, "", 100).ok, "buy phoenix")
				else:
					if not (route == "natural" and earn(s, v)): check(s.counter_command("reject", v.visit_id).ok, "ordinary transaction ends")
				calm(s)
			else:
				if progress(s): continue
				if route == "natural" and sell_goods(s): continue
	...
	if begin == 11 and route != "defer": check(finish_night <= 17, "finish by night17")
	verify(s, route)
...
func progress(s: RunSession) -> bool:
	...
	if route not in ["pay", "sell_pay"]: steps.append(["fd_search_motive", "later" if route == "defer" and FirstDebt.last(st, "fd_search_motive").is_empty() else "help"])
	...
	if route in ["pay", "sell_pay", "sold_search"]: steps.append(["fd_compensation", "offer"])
	steps.append(["fd_settle", "pay" if route in ["pay", "sell_pay", "sold_search"] else "return"])
	steps.append(["fd_followup", "listen"])
	for pair in steps:
		if route == "sold_search" and pair[0] in ["fd_compensation", "fd_settle"] and FirstDebt.owned(st, FirstDebt.DRAGON) == null: continue
...
func fixture(s: RunSession, stage: String) -> void:
	if route not in ["return", "natural"]: return
	var directory := "res://.godot/qa/v31-natural/" if route == "natural" else "res://.godot/qa/v31/"
...
func verify(s: RunSession, label: String) -> void:
	...
	check(codec.decode(codec.encode(s._day.state, 31), run_def, 31, catalog, true) != null, "v31 replay " + label + " " + codec.error_message)
