...
func _run() -> void:
	...
	var item_button := screen.get_node("CounterView/CounterItemImage") as TextureButton
	_check(item_button.texture_normal != null, "柜台显示青花碗独立轮廓")
	var before := _session.read_state()
	await _click_control(item_button)
	var appraisal := _main.find_child("AppraisalPanel", true, false) as AppraisalPanel
	_check(appraisal.is_visible_in_tree(), "点击柜台物品打开鉴定")
	...
	_check(appraisal._images.size() == 3 and appraisal._selected == "detail_repair", "侧光取证后才出现并选中接缝局部图")
	...
	_check(item_button.texture_normal == null and not item_button.visible, "成交后清除旧物品图像")
	await _capture("07_purchase")
	await _wait_to(90)
	await _click("收起 · Esc")
	_check(item_button.texture_normal.resource_path.contains("holder_front"), "下一客切换为烛台，没有旧物品残留")
	...
	_check(appraisal._selected == "detail_iron_core", "取得划痕后显示灰芯局部图")
