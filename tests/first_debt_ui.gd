...
func _run() -> void:
	...
	aqi_manifest = "res://data/first_debt_manifest.json"; aqi_version = 28; aqi_fixture_dir = "res://.godot/qa/v28/"
	...
	_main.get_node("Bootstrap").save_path = "res://.godot/qa/v28/ui-unused.json"
	root.add_child(_main)
	_session = _main.get_node("Bootstrap").session
	_check(_session != null and _session.content_version == 28, "production v28 default")
	_session._save.library.path = "res://.godot/qa/v28/ui-library-%d.json" % Time.get_ticks_usec()
	...
	for b in buttons: _check(not b.label.contains("清") and not b.label.contains("和解"), "no outcome stamp before choice")
	...
	for route in ["return", "pay", "late25"]:
		...
		await _click("付300银元，与陈家议妥 · 5分钟" if route == "pay" else "将两只金镯交还陈家 · 5分钟")
...
func shot(stage: String) -> void:
	...
	var path := "res://docs/qa/first-debt-v28/%d-%s.png" % [root.size.x, stage]
