...
func _run() -> void:
	...
	_check(_session.content_version == 20 and _session.definition.id == "pawn_chance_seven" and FamiliarStories.enabled(_session.definition) and not _session.definition.market.is_empty(), "default combines familiar stories with market")
