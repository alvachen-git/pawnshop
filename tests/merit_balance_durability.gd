extends "res://tests/merit_balance_recovery_durability.gd"
...
func run() -> void:
	...
	for mode in ["return", "pay"]:
		...
		var lib := SaveLibrary.new("res://.godot/qa/v43/merit-" + mode + ".json")
		...
		check(lib.write_entry("auto/" + String(run_def.id), s._day.state, run_def, 43, catalog), "initial save")
		...
		var ledger_probe := RunSession.new(run_def,43,ledger_store,catalog)
		...
		for forged in [-101, 101, 20, 0, 9.5, "10"]:
			var raw := codec.encode(s._day.state, 43); raw.hidden_merit = forged
			check(codec.decode(raw, run_def, 43, catalog, true) == null, "reject merit tamper " + str(forged))
		var raw := codec.encode(s._day.state, 43)
		var file := FileAccess.open("res://.godot/qa/v43/merit-pending-" + mode + ".json", FileAccess.WRITE); file.store_string(JSON.stringify(raw)); file.close()
		var restored := codec.decode(raw, run_def, 43, catalog, true)
		...
		raw = codec.encode(s._day.state, 43)
		file = FileAccess.open("res://.godot/qa/v43/merit-seen-" + mode + ".json", FileAccess.WRITE); file.store_string(JSON.stringify(raw)); file.close()
		# Deleting either acknowledgment journal or history cannot forge playback.
		raw.action_journal.pop_back()
		check(codec.decode(raw, run_def,43,catalog,true) == null, "forged echo rejected")
	...
	var fresh := RunSession.new(run_def,43,fresh_store,catalog)
