...
func test_manifest() -> String:
	return "res://data/first_debt_merit_balance_manifest.json"

func verify(s: RunSession, label: String) -> void:
	check(s._day.state.hidden_merit == ((5 if FirstDebt.flag(s._day.state, "fd_peace") else 10) if FirstDebt.settled(s._day.state) else 0), "merit tracks settlement only " + label)
...
func compensation_ready(s: RunSession) -> bool:
	return route != "natural_pay" or s._day.state.cash >= 340
