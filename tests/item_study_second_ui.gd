...
const CASES := [["item_inkstone", "sound"], ["item_clay_teapot", "sound"], ["item_silk_panel", "sound"], ["item_pocket_watch", "sound"], ["item_pocket_watch", "flawed"], ["item_pocket_watch", "worn"], ["item_folding_fan", "sound"], ["item_folding_fan", "flawed"], ["item_folding_fan", "mended"]]
...
func _run() -> void:
	...
	for case in CASES:
		...
		if "fan-only" in OS.get_cmdline_user_args() and id != "item_folding_fan": continue
		var chosen := -1
		var target := ""
		var night := 1
		...
		_main.get_node("Bootstrap").save_path = "res://.godot/qa/item-studies-second/runtime/auto_%d.json" % Time.get_ticks_usec()
		root.add_child(_main)
		_session = _main.get_node("Bootstrap").session
		if _session._save.library != null: _session._save.library.path = "res://.godot/qa/item-studies-second/runtime/library_%d.json" % Time.get_ticks_usec()
		...
		_check(id == "item_folding_fan" or not visit.item.revealed_clue_ids.is_empty(), "real evidence actions remain available " + id)
		var expected := ["front", "back"]
		if id == "item_pocket_watch" and variant in ["sound", "flawed"]: expected.append(variant)
		...
		if price <= cash:
			...
			_check(receipt.visible and receipt._picture.texture.resource_path == CounterItemArt.front_path(asset), "receipt uses same painted object")
			await _capture(id + "_receipt")
			await receipts()
			...
			await _click("库存"); await _frames()
			var inventory := _main.find_child("InventoryPanel", true, false)
			_check(_contains_texture(inventory, CounterItemArt.front_path(asset)), "actual inventory uses painted front " + id)
			await _capture(id + "_inventory")
		_main.queue_free(); await process_frame
	print("SECOND ITEM STUDIES UI: %d assertions, %d failures" % [_assertions, _failures])
	quit(0 if _failures == 0 else 1)

func _capture(label: String) -> void:
	if DisplayServer.get_name() == "headless": return
	await RenderingServer.frame_post_draw
	var folder := "res://.godot/qa/item-studies-second/"
...
func _contains_texture(node: Node, path: String) -> bool:
	if node is TextureRect and node.is_visible_in_tree() and node.texture != null and node.texture.resource_path == path: return true
	for child in node.get_children():
		if _contains_texture(child,path): return true
	return false
