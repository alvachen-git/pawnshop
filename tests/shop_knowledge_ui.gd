...
func _run() -> void:
	...
	var button := _find_button(room,"学习顾砚生知识 · 准备1次")
	...
	store.fail = false; await _click("学习顾砚生知识 · 准备1次")
	...
	_check(_find_button(room,"学习顾砚生知识 · 准备1次") == null and room.body.text.contains("已掌握"), "learned replaces study action")
	...
	_check(_find_button(room,"研习扇画图录 · 准备1次 / 不收费") == null, "bench no longer teaches generic knowledge")
