...
func run() -> void:
	...
	var report := []
	for stage in ["ordinary-ready","ordinary-pressed","saved-sale"]:
		var payload: Dictionary=JSON.parse_string(FileAccess.get_file_as_string(DIR+stage+".json"))
	...
	DirAccess.make_dir_recursive_absolute("res://docs/qa/fan-bargaining")
	FileAccess.open("res://docs/qa/fan-bargaining/performance.json",FileAccess.WRITE).store_string(JSON.stringify(report,"\t"))
	print("FAN BARGAINING PERFORMANCE: %d passes, %d failures; %s" % [passes,failures,JSON.stringify(report)])
