...
func setup() -> bool:
	...
	check(loaded.is_success(),"v48 journey catalog")
...
func verify(s:RunSession,label:String) -> void:
	...
	var codec:=SaveCodec.new();var restored:=codec.decode(codec.encode(s._day.state,48),run_def,48,catalog,true)
	check(restored!=null,"v48 cold replay "+label+" "+codec.error_message)
...
func run() -> void:
	...
	check(gramophone_claims>0,"v48 natural gramophone negotiation and saved actions")
	print("GRAMOPHONE V48 JOURNEY: %d passes, %d failures" % [passes,failures]);quit(0 if failures==0 else 1)
