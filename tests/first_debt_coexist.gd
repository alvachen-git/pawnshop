...
var redeemed := false
func run() -> void:
	reunion_manifest = "res://data/first_debt_manifest.json"
	reunion_fixture_dir = "res://.godot/qa/v28-coexist/"
		...
		for step in 200:
			...
			if v == null:
				...
				if n == 12 and not FirstDebt.flag(s._day.state, "fd_family_read"):
					for pair in [["fd_family", "read"], ["fd_dragon", "buy"], ["fd_truth", "tell"], ["fd_settle", "return"]]: check(s.event_command(pair[0], pair[1]).ok, "coexist resolution " + pair[0])
...
func risk_after_case() -> void:
	...
	var state := SaveCodec.new().decode(raw, run_def, 28, catalog, true)
