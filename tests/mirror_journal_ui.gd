...
func run() -> void:
	...
	main.get_node("Bootstrap").manifest_path = "res://data/mirror_investigation_manifest.json"
	...
	var latest := JsonContentProvider.new("res://data/aqi_investigation_manifest.json").load_catalog()
	var store := GhostReplayStore.new()
	store.origin = {"seed": 42, "run_token": "0123456789abcdef0123456789abcdef"}
	var latest_session := RunSession.new(latest.catalog.get_definition("runs", latest.catalog.default_run_id), latest.catalog.content_version, store, latest.catalog)
	latest_session._day.state.event_history.append({"event_id": "aq_coat", "choice_id": "look", "night": 7})
	check(preload("res://ui/risk/mirror_journal.gd").build(latest_session._day, latest.catalog).is_empty(), "v24 room discoveries do not leak into mirror evidence")
