...
func _run() -> void:
	...
	var frame_texture := mirror.frame.texture
	var frame_bounds := mirror.frame.get_global_rect()
	...
	check(mirror.frame.texture == frame_texture and mirror.frame.get_global_rect() == frame_bounds, "熄灯保留镜框素材与位置")
	check(dark.get_pixelv(frame_point).get_luminance() <= normal.get_pixelv(frame_point).get_luminance(), "镜框间接光随命灯减弱")
	...
	mirror.set_state({"mode": "shadow", "strength": 0.82})
	var shadow := await shot("shadow")
	var shadow_point := Vector2i(surface.position + surface.size * Vector2(0.79, 0.5))
	check(shadow.get_pixelv(shadow_point).get_luminance() < normal.get_pixelv(shadow_point).get_luminance() * 0.5, "墙角影子实际变暗")
	check(shadow.get_region(outside).get_data() == normal.get_region(outside).get_data() and shadow.get_pixelv(frame_point) == normal.get_pixelv(frame_point), "影子不改变镜框及房间其它区域")
	...
	check(mirror._surface.get_shader_parameter("wall_shadow") == 0.0, "切换异象清除墙角影子")
