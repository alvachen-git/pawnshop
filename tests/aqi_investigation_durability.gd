...
func run() -> void:
	...
	run_def = catalog.get_definition("runs", "aqi_investigation_ten")
	for stage in ["aq-room-2", "aq-example-aq_arrival"]:
		var codec := SaveCodec.new()
		var raw = JSON.parse_string(FileAccess.get_file_as_string("res://.godot/qa/v24/" + stage + ".json"))
		var state := codec.decode(raw, run_def, 24, catalog)
		...
		s._save.library = lib
		check(lib.write_entry("auto/aqi_investigation_ten", state, run_def, 24, catalog), "baseline aqi auto slot")
		check(lib.write_entry("manual/1", state, run_def, 24, catalog), "same shared manual slot")
		var bytes := FileAccess.get_file_as_bytes(lib.path)
		var before := s.read_state()
		lib.fail_write = true
		var observation: bool = stage.contains("room-2")
		...
		check(not lib.read_entry("auto/aqi_investigation_ten").is_empty(), "previous checkpoint remains readable")
		...
		var read := lib.read_entry("auto/aqi_investigation_ten")
	...
	print("AQI V24 DURABILITY: %d passes, %d failures" % [passes, failures])
