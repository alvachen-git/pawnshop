...
func run() -> void:
	...
	for channel in ["single","batch"]:
		var s:=camera("original","clear","smooth");var v:CustomerVisit=s._day.state.visits[0]
		var o:=CameraEconomy.owner(s._day.state,v);o.belief={"identity":"imitation","lens":"haze","mechanism":"sticky"};o.rate=60
		CameraEconomy.reprice_initial(s._day.state,v)
	...
	for redeem in [false,true]:
		var s:=camera("rebuilt","haze","sticky","minor","customer_wealthy_antique","pawn");var v:CustomerVisit=s._day.state.visits[0]
		...
		if redeem:
			...
			check(offer==roundi(fixed.camera_value.actual*.9),"display uses actual value after default")
	...
	var poor:=camera();var visitor:CustomerVisit=poor._day.state.visits[0];poor._day.state.cash=0;var snapshot:=poor.read_state()
	...
	print("CAMERA ECONOMY: %d passes, %d failures" % [passes,failures]);quit(0 if failures==0 else 1)
