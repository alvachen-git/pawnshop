...
func _waiting_and_deadlines() -> void:
	...
	check.call(deadline.counter_command("offer", id, "", 60).ok and deadline.read_state().cash == 40 and deadline.read_state().inventory_instances.size() == 1, "离场前提交的合适报价，完成恰到期限仍成交。")
	check.call(deadline.read_state().game_minutes == 100 and _outcome(deadline, id) == "bought", "成交照常消耗报价时间，记录收货而非超时离场。")
	var closed := _session()
	id = _open(closed)
