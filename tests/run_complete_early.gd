...
func run() -> void:
	...
	check(loaded.is_success(), "content19")
	...
	for route in ["allow", "defer", "miss", "deadline", "short"]:
		play(chosen, route)
		if failures > 0: break
	print("COMPLETE EARLY: %d passes, %d failures" % [passes, failures])
...
func roundtrip(s: RunSession, label: String) -> void:
	...
	var dir := "res://.godot/qa/complete_early"
...
func play(seed_value: int, route: String) -> void:
	...
	var save := SaveManager.new("user://tests/complete_early/%s.json" % route)
	var s := RunSession.new(run_def, 19, save, catalog)
	s.new_run()
	...
	for n in range(1, 8):
		...
		for step in 180:
			...
			if route == "deadline":
				...
				var flow := CashFlowReadModel.build(s._day.state, run_def)
				check(flow.cash == before + (ticket.redemption_amount if route == "allow" else 0) and flow.pawn_principal == (0 if route == "allow" else ticket.principal), "early return updates cash and tied principal only after acceptance")
