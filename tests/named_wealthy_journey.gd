...
func verify(s: RunSession, label: String) -> void:
	...
	var codec := SaveCodec.new(); var restored := codec.decode(codec.encode(s._day.state,37),run_def,37,catalog,true)
	check(restored != null,"v37 cold replay "+label+" "+codec.error_message)
	if restored != null: check(GhostSaveCodec.same(restored.to_read_model(),s.read_state()),"exact v37 replay "+label)
