...
func _run() -> void:
	...
	aqi_version = 28
	aqi_fixture_dir = "res://docs/qa/first-debt-v28/fixtures/"
	root.size = Vector2i(1600, 900) if "wide" in OS.get_cmdline_user_args() else Vector2i(1280, 720)
	root.content_scale_size = root.size
	root.always_on_top = true
	root.title = "旧当铺 · 册页验收"
	_main = load("res://scenes/start.tscn").instantiate()
	_main.get_node("Bootstrap").manifest_path = "res://data/first_debt_manifest.json"
	_main.get_node("Bootstrap").save_path = "res://.godot/qa/v28/album-unused.json"
	root.add_child(_main)
	_session = _main.get_node("Bootstrap").session
	_session._save.library.path = "res://.godot/qa/v28/album-library-%d.json" % Time.get_ticks_usec()
	...
	var saved := _session._save.library.read_entry("auto/first_debt_open")
...
func shot(stage: String) -> void:
	...
	_check(root.get_texture().get_image().save_png(("res://docs/qa/old-shop-refinement/" if "refinement" in OS.get_cmdline_user_args() else "res://docs/qa/old-shop-album/") + ("%d-%s.png" % [root.size.x, stage])) == OK, "capture " + stage)
