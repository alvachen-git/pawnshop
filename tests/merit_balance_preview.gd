...
func run() -> void:
	...
	var okay := s.content_version == 43 and s.definition.initial_cash == 300 and main.title_menu == null and s._day.state.hidden_merit == 0
	okay = okay and s._save.library.path.begins_with("user://tests/v43_preview/")
	...
	okay = okay and action.ok and s._day.state.hidden_merit == (5 if mode == "pay" else 10) and s._day.state.cash == before - (300 if mode == "pay" else 0)
