...
func run() -> void:
	...
	print("COMPLETE FAMILIAR TESTS: %d passes, %d failures" % [passes, failures])
	quit(0 if failures == 0 else 1)

func roundtrip(s: RunSession, label: String) -> void:
	var codec := SaveCodec.new()
	var data := codec.encode(s._day.state, 19)
	var restored := codec.decode(JSON.parse_string(JSON.stringify(data)), run_def, 19, catalog, true)
	check(restored != null, "restore " + label + ": " + codec.error_message)
	var folder := "res://.godot/qa/complete_familiar/cross"
...
func play(seed_value: int, route: String) -> void:
	...
	var saves := SaveManager.new("res://.godot/qa/complete_familiar/%s_%d.json" % [route, seed_value])
	var s := RunSession.new(run_def, 19, saves, catalog)
	for n in range(1, 8):
		driver.drain(s)
	...
	roundtrip(s, "final")
	var data := SaveCodec.new().encode(s._day.state, 19)
	var file := FileAccess.open("res://.godot/qa/complete_familiar/%s_%d_final.json" % [route, seed_value], FileAccess.WRITE)
	file.store_string(JSON.stringify(data)); file.close()
	var bad := data.duplicate(true)
	bad.familiar_plan.stories[0].follow_night += 1
	check(SaveCodec.new().decode(bad, run_def, 19, catalog, true) == null, "reject altered schedule")
	bad = data.duplicate(true); bad.familiar_progress.clear()
	check(SaveCodec.new().decode(bad, run_def, 19, catalog, true) == null, "reject altered progress")
...
func seamstress(s: RunSession, v: CustomerVisit, row: Dictionary, route: String) -> void:
	if EarlyRedemption.is_visit(v): command(s, v, "early_redeem"); return
