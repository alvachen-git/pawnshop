...
func run() -> void:
	...
	check(s._day.state.cash == 1000 and s._day.state.fee_history.size() == 100, "100 daily fees, no night21 principal")
	...
	verify(s, "ended100")
	print("FIRST DEBT LONG: %d passes, %d failures" % [passes, failures])
