...
static func finish(state: RunState, visit: CustomerVisit, outcome: String) -> void:
	...
	if (WatchEconomy.handles(state,visit.item) or PearlEconomy.handles(state,visit.item) or PorcelainEconomy.handles(state,visit.item) or BangleEconomy.handles(state,visit.item)) and change < 0: change = 0
