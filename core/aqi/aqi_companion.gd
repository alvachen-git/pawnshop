...
static func model(day: DayController, events: EventDirector, counter: CounterService, mirror_busy: bool) -> Dictionary:
	...
	var present := n >= 8 and n <= 10 and s.phase == &"open" and s.game_minutes < 240 and "aq_seated" in flags and "aq_left_%d" % n not in flags and "aq_refused_%d" % n not in flags
	...
	for topic in ["chat", "paper", "leave"]:
		...
		for row in s.event_history:
			if row.event_id == id: chosen = row.choice_id
