...
func initialize() -> ContentLoadResult:
	...
	if result.is_success():
		...
		if not preview_stage.is_empty():
			var fixture_path := "res://docs/qa/first-debt-v%d/fixtures/" % preview_version if FirstDebt.enabled(definition) else "res://.godot/qa/v%d/" % preview_version
			var data: Variant = JSON.parse_string(FileAccess.get_file_as_string(fixture_path + preview_stage + ".json"))
			var codec := SaveCodec.new()
