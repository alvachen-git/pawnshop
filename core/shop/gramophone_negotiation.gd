class_name GramophoneNegotiation
extends RefCounted

const PARTS := ["identity","sound","motor","exterior"]
const LABELS := {"identity":"身份与部件","sound":"发声状态","motor":"转速与动力","exterior":"外观"}
const OPTIONS := {
 "identity":{"original":"原装胜利牌","rebuilt":"拼配或改装","imitation":"仿制或冒名"},
 "sound":{"clear":"声音清楚","rasping":"持续破音","muffled":"发声闷弱"},
 "motor":{"steady":"转速平稳","wavering":"转速忽快忽慢","stopping":"上弦后仍停转"},
 "exterior":{"observed":"按查出的外伤谈价"}}

static func data(state: RunState, visit: CustomerVisit) -> Dictionary:
	return WealthyCustomers.trade(state,visit).get("gramophone_negotiation",{})
...
static func prepare(state: RunState, visit: CustomerVisit) -> void:
	var n := VarietyService.rng(state.run_seed,visit.visit_id+"/gramophone44/personality").randi_range(0,99)
	var rolls := {}
	for part in PARTS: rolls[part] = VarietyService.rng(state.run_seed,visit.visit_id+"/gramophone44/claim/"+part).randi_range(0,99)
	WealthyCustomers.trade(state,visit)["gramophone_negotiation"] = {"personality":"easy" if n<35 else "careful" if n<80 else "firm","rolls":rolls,"claims":{},"facts":{},"responses":[],"penalized":false,"belief":GramophoneEconomy.owner(state,visit).belief.duplicate(true),"initial_asking":visit.trade.asking_price,"initial_reserve":visit.trade.reserve_price}

static func strength(state: RunState, visit: CustomerVisit, part: String) -> int:
	var d:=GramophoneAppraisal.row(state,visit.item.instance_id)
	if part=="exterior":return 2
	if part=="identity":return 2 if d.get("identity_zoom",false) and "soundbox" in d.get("viewed",[]) else 1 if "identity" in d.get("viewed",[]) else 0
	var tests: Array=d.get("tests",[])
	if tests.is_empty():return 0
	var wound:=false;var reference:=false;var customer:=false
	for entry in tests:
		wound = wound or String(entry).begins_with("wound/")
		reference = reference or entry=="wound/reference/nominal"
		customer = customer or entry=="wound/customer/nominal"
	return 2 if reference and (customer if part=="sound" else d.get("speeds",[]).size()>=2) else 1 if wound else 0

static func truth(item: ItemInstance, part: String) -> String:
	return "observed" if part=="exterior" else String(item.goods.gramophone_value[part])
...
static func chance(state: RunState, visit: CustomerVisit, part: String, claim: String) -> int:
	...
	var skill := ["expert","ordinary","novice"].find(GramophoneEconomy.owner(state,visit).skill)
...
static func reason(day: DayController, visit: CustomerVisit, detail: String, amount: int) -> String:
	if visit == null or not GramophoneEconomy.handles(day.state,visit.item): return "眼下没有这笔留声机生意。"
	...
	for part in claims:
		...
		if part in ["motor","sound"] and GramophoneAppraisal.row(day.state,visit.item.instance_id).get("tests",[]).is_empty(): return "先点击试听，再谈运转或发声。"
...
static func concession(state: RunState, visit: CustomerVisit) -> float:
	var urgent: bool = GramophoneEconomy.owner(state,visit).urgent
...
static func reprice(state: RunState, visit: CustomerVisit) -> Dictionary:
	var d := data(state,visit); var o := GramophoneEconomy.owner(state,visit); var trade := WealthyCustomers.trade(state,visit)
	var value := GramophoneEconomy.valuation(d.belief.identity,d.belief.sound,d.belief.motor,visit.item.goods.precision.damage)
	...
	trade.value = GramophoneEconomy.valuation(d.facts.get("identity","original"),d.facts.get("sound","clear"),d.facts.get("motor","steady"),visit.item.goods.precision.damage if d.facts.has("exterior") else "intact")
...
static func unchanged_reply(d: Dictionary, pricing: Dictionary, before: int, belief_before: Dictionary, results: Dictionary) -> String:
	...
	if pricing.fraction == 0.0:
		return "客人拢起留声机：‘您说的货况我认。不过这架留声机，我仍要这个价，少了便先留着。’"
...
static func model(day: DayController, visit: CustomerVisit) -> Dictionary:
	var d := data(day.state,visit); var r := GramophoneAppraisal.row(day.state,visit.item.instance_id)
	var defaults := {"identity":r.get("identity","unsure"),"sound":r.get("sound","unsure"),"motor":r.get("motor","unsure"),"exterior":"observed" if TieredAppraisal.record(day.state,visit.item.instance_id).get("exterior",false) else "unsure"}
	var rows := []
	for part in PARTS:
...
static func devalues(item: ItemInstance, part: String, claim: String) -> bool:
	...
	var table: Dictionary={"identity":GramophoneEconomy.BASE,"sound":GramophoneEconomy.SOUND,"motor":GramophoneEconomy.MOTOR}[part]
