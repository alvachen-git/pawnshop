class_name PorcelainNegotiation
extends RefCounted

const PARTS := ["era","craft","exterior"]
const LABELS := {"era":"年代判断","craft":"工艺判断","exterior":"外观"}
const OPTIONS := {
...
static func data(state: RunState, visit: CustomerVisit) -> Dictionary:
	return WealthyCustomers.trade(state,visit).get("porcelain_negotiation",{})
...
static func prepare(state: RunState, visit: CustomerVisit) -> void:
	var n := VarietyService.rng(state.run_seed,visit.visit_id+"/porcelain41/personality").randi_range(0,99)
	var rolls := {}
	for part in PARTS: rolls[part] = VarietyService.rng(state.run_seed,visit.visit_id+"/porcelain41/claim/"+part).randi_range(0,99)
	WealthyCustomers.trade(state,visit)["porcelain_negotiation"] = {"personality":"easy" if n<35 else "careful" if n<80 else "firm","rolls":rolls,"claims":{},"facts":{},"responses":[],"penalized":false,"belief":PorcelainEconomy.owner(state,visit).belief.duplicate(true),"initial_asking":visit.trade.asking_price,"initial_reserve":visit.trade.reserve_price}

static func strength(state: RunState, visit: CustomerVisit, part: String) -> int:
	var groups: Array = PorcelainAppraisal.row(state,visit.item.instance_id).get("viewed",[])
	if part == "exterior": return 2
	if part == "era": return 2 if groups.size()==3 else 1 if not groups.is_empty() else 0
	var count := int("painting" in groups)+int("foot" in groups)
	return count

static func truth(item: ItemInstance, part: String) -> String:
	return PorcelainEconomy.era(item) if part == "era" else PorcelainEconomy.craft(item) if part == "craft" else "observed"
...
static func chance(state: RunState, visit: CustomerVisit, part: String, claim: String) -> int:
	...
	var skill := ["expert","ordinary","novice"].find(PorcelainEconomy.owner(state,visit).skill)
...
static func reason(day: DayController, visit: CustomerVisit, detail: String, amount: int) -> String:
	if visit == null or not PorcelainEconomy.handles(day.state,visit.item): return "眼下没有这笔瓷瓶生意。"
...
static func concession(state: RunState, visit: CustomerVisit) -> float:
	var urgent: bool = PorcelainEconomy.owner(state,visit).urgent
...
static func reprice(state: RunState, visit: CustomerVisit) -> Dictionary:
	var d := data(state,visit); var o := PorcelainEconomy.owner(state,visit); var trade := WealthyCustomers.trade(state,visit)
	var value := PorcelainEconomy.valuation(d.belief.era,d.belief.craft,visit.item.goods.precision.damage)
	...
	trade.value = PorcelainEconomy.valuation(d.facts.get("era","yuan"),d.facts.get("craft","fine"),visit.item.goods.precision.damage if d.facts.has("exterior") else "intact")
...
static func unchanged_reply(d: Dictionary, pricing: Dictionary, before: int, belief_before: Dictionary, results: Dictionary) -> String:
	...
	if d.belief == belief_before:
		if results.has("craft") and results.craft.accepted and results.craft.claim == "rough":
			return "客人指了指瓶身：‘这工我原就按粗工算了，开的也是这个价。不能再为同一处折一回。’"
	...
	if pricing.fraction == 0.0:
		return "客人拢起瓷瓶：‘您说的货况我认。不过这只瓶子，我仍要这个价，少了便先留着。’"
...
static func model(day: DayController, visit: CustomerVisit) -> Dictionary:
	var d := data(day.state,visit); var r := PorcelainAppraisal.row(day.state,visit.item.instance_id)
	var defaults := {"era":r.get("era","unsure"),"craft":r.get("craft","unsure"),"exterior":"observed" if TieredAppraisal.record(day.state,visit.item.instance_id).get("exterior",false) else "unsure"}
	var rows := []
	for part in PARTS:
...
static func devalues(item: ItemInstance, part: String, claim: String) -> bool:
	if part == "era": return PorcelainEconomy.BASE[claim] < PorcelainEconomy.BASE[truth(item,part)]
	if part == "craft": return PorcelainEconomy.CRAFT[claim] < PorcelainEconomy.CRAFT[truth(item,part)]
	return false
