...
static func reprice(state: RunState, visit: CustomerVisit) -> void:
	...
	var target_reserve := maxi(int(row.funding),roundi(reference*LuxuryCarry.percent(state,visit,"reserve")*factor))
	var target_asking := maxi(target_reserve,roundi(reference*LuxuryCarry.percent(state,visit,"asking")*factor))
