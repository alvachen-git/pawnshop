class_name PearlReadModels
extends RefCounted

static func enrich(model: Dictionary, day: DayController, visit: CustomerVisit) -> void:
	var item := visit.item; var record := TieredAppraisal.record(day.state,item.instance_id); var r := PearlAppraisal.row(day.state,item.instance_id)
	var factor := float(TieredAppraisal.RETAIN[item.goods.precision.damage])/100.0 if record.get("exterior",false) else 1.0
	var estimate := "%d–%d" % [roundi(60*factor),roundi(440*factor)]
	var notes := PackedStringArray()
	for part in ["material","repair"]:
		if r.get(part,"unsure") != "unsure": notes.append(PearlNegotiation.OPTIONS[part][r[part]])
	...
	model.trade["pearl_claims"] = PearlNegotiation.model(day,visit)
