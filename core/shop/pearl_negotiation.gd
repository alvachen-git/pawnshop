class_name PearlNegotiation
extends RefCounted

const PARTS := ["material","repair","exterior"]
const LABELS := {"material":"仿珠情况","repair":"真珠品质","exterior":"外观"}
const OPTIONS := {
	"material":{"none":"未见混仿","few":"混入少量仿珠","some":"混入部分仿珠","most":"多数为仿珠","all":"整串仿珠"},
	"repair":{"intact":"未见低档换配","altered":"疑有低档真珠换入"},
	"exterior":{"observed":"按查出的外观谈价"}}

static func data(state: RunState, visit: CustomerVisit) -> Dictionary:
	return WealthyCustomers.trade(state,visit).get("pearl_negotiation",{})
...
static func prepare(state: RunState, visit: CustomerVisit) -> void:
	var n := VarietyService.rng(state.run_seed,visit.visit_id+"/pearl38/personality").randi_range(0,99)
	var rolls := {}
	for part in PARTS: rolls[part] = VarietyService.rng(state.run_seed,visit.visit_id+"/pearl38/claim/"+part).randi_range(0,99)
	WealthyCustomers.trade(state,visit)["pearl_negotiation"] = {"personality":"easy" if n<35 else "careful" if n<80 else "firm","rolls":rolls,"claims":{},"facts":{},"responses":[],"penalized":false,"belief":PearlEconomy.owner(state,visit).belief.duplicate(true),"initial_asking":visit.trade.asking_price,"initial_reserve":visit.trade.reserve_price}

static func strength(state: RunState, visit: CustomerVisit, part: String) -> int:
	var r := PearlAppraisal.row(state,visit.item.instance_id)
	if part == "exterior": return 2
	if part == "repair": return mini(2,r.get("pairs",[]).size())
	var count: int = r.get("holes",[]).size()
	return 0 if count == 0 else 1 if count < 3 else 2

static func truth(item: ItemInstance, part: String) -> String:
	return PearlEconomy.material(item) if part == "material" else PearlEconomy.repair(item) if part == "repair" else "observed"
...
static func chance(state: RunState, visit: CustomerVisit, part: String, claim: String) -> int:
	...
	var skill := ["expert","ordinary","novice"].find(PearlEconomy.owner(state,visit).skill)
...
static func reason(day: DayController, visit: CustomerVisit, detail: String, amount: int) -> String:
	if visit == null or not PearlEconomy.handles(day.state,visit.item): return "眼下没有这笔珠串生意。"
...
static func concession(state: RunState, visit: CustomerVisit) -> float:
	var urgent: bool = PearlEconomy.owner(state,visit).urgent
...
static func reprice(state: RunState, visit: CustomerVisit) -> void:
	var d := data(state,visit); var o := PearlEconomy.owner(state,visit); var trade := WealthyCustomers.trade(state,visit)
	var value := PearlEconomy.valuation(d.belief.material,d.belief.repair,visit.item.goods.precision.damage)
	...
	trade.value = PearlEconomy.valuation(d.facts.get("material","none"),d.facts.get("repair","intact"),visit.item.goods.precision.damage if d.facts.has("exterior") else "intact")
...
static func submit(day: DayController, visit: CustomerVisit, detail: String) -> String:
	...
	if accepted: replies.append("客人想了想：‘照这个价谈吧。’" if after < before and concession(day.state,visit)==1.0 else "客人摇头：‘只能再让这些。’" if after < before else "客人拢起珠串：‘货况认下，价钱已让到头了。’")
...
static func model(day: DayController, visit: CustomerVisit) -> Dictionary:
	var d := data(day.state,visit); var r := PearlAppraisal.row(day.state,visit.item.instance_id)
