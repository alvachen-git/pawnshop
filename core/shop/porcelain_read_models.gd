class_name PorcelainReadModels
extends RefCounted

static func enrich(model: Dictionary, day: DayController, visit: CustomerVisit) -> void:
	var item := visit.item; var record := TieredAppraisal.record(day.state,item.instance_id); var r := PorcelainAppraisal.row(day.state,item.instance_id)
	var factor := float(TieredAppraisal.RETAIN[item.goods.precision.damage])/100.0 if record.get("exterior",false) else 1.0
	var estimate := "%d–%d" % [roundi(54*factor),roundi(1260*factor)]
	var notes := PackedStringArray()
	for part in ["era","craft"]:
		if r.get(part,"unsure") != "unsure": notes.append(PorcelainNegotiation.OPTIONS[part][r[part]])
	for visual: Dictionary in [model.visual,model.appraisal.visual,model.dialogue.visual,model.trade.visual]:
		visual["porcelain_art"] = item.goods.porcelain_value.duplicate(true)
		visual.porcelain_art.erase("actual")
		visual.porcelain_art["damage"] = item.goods.precision.damage
	...
	model.trade["porcelain_claims"] = PorcelainNegotiation.model(day,visit)
