...
static func build(state: RunState) -> Dictionary:
	...
	result.operating_profit = result.realized_profit - result.interest_expense - result.shop_expense - int(result.get("provenance_expense", 0)) - int(result.get("preparation_expense", 0))
	return result
