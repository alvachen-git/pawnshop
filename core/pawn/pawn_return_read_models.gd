...
static func enrich(model: Dictionary, day: DayController, service: CommerceService) -> void:
	...
	var description := "当票 %s · %s\n当户：%s\n本金 %d 银元 · 约定赎金 %d 银元\n办理 %d 分钟。原物仍在铺内，验票后交还。" % [number, definition.display_name, customer.terms.display_name, ticket.principal, ticket.redemption_amount, visit.minutes]
	if visit.command == "extend": description = "当票 %s · %s\n当户：%s\n续当费 %d 银元 · 延长 %d 夜\n办理 %d 分钟，原物继续留铺。" % [number, definition.display_name, customer.terms.display_name, ceili(ticket.principal * terms.extension_fee_ratio), terms.extension_nights, visit.minutes]
	...
	model.visual = {"pawn_return": true, "customer_name": customer.terms.display_name, "portrait_asset": customer.portrait_asset_id,
