...
static func validate(catalog: ContentCatalog) -> Array:
	...
	for event: EventDefinition in catalog.get_all("events"):
		if event.kind not in ["anchor", "conditional", "random"] or event.phase not in ["pre_open", "open", "closed_processing"] or event.night_min < 1 or event.night_max < event.night_min or event.window_start < 0 or event.window_end <= event.window_start or event.priority < 0 or event.weight < 1 or event.max_count < 1 or event.cooldown < 0 or event.choices.is_empty():
			_error(issues, event.id, "事件类型、窗口、权重、次数或选项无效。")
		# M4 guaranteed anchors run before opening, where player actions cannot skip them.
		if event.kind == "anchor" and (event.phase != "pre_open" or event.window_start != 0 or not event.required_flags.is_empty() or not event.excluded_flags.is_empty() or not event.required_items.is_empty() or not event.conflicts_with.is_empty()):
		...
		for choice in event.choices:
			if choice.id.is_empty() or choice.id in choices or choice.minutes < 0 or (event.phase == "pre_open" and choice.minutes != 0) or (event.phase != "pre_open" and choice.minutes == 0): _error(issues, event.id, "选项ID重复或耗时不符合阶段。")
	...
	for run: RunDefinition in catalog.get_all("runs"):
		...
		for id in run.event_ids:
			...
			if event == null:
				...
				continue
			if event.night_min > run.total_nights or event.window_end > run.night_minutes or event.window_start % run.time_step != 0 or event.window_end % run.time_step != 0: _error(issues, id, "事件窗口超出运行或不匹配步长。")
