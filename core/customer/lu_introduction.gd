...
const ALL_EVENTS := ["lu_visit_greeting", "lu_visit_trade", "lu_visit_letters", "lu_pawn_collateral", "lu_pawn_interest", "lu_pawn_maturity"]
...
static func active(state: RunState) -> bool:
	return state.phase == &"pre_open" and state.pending_event_id in ALL_EVENTS and not MilitaryIntroduction.active(state)
...
static func model(base: Dictionary, day: DayController, catalog: ContentCatalog) -> Dictionary:
	...
	base.visual.introduction = ["新掌柜吧？敝姓陆，来认认人。", "替各家商号跑货，也替老朋友寻个买主。", "收货的需求变了，我便托人送信来。", "今天带了当票来，说说活当的规矩。", "本金、息钱，都得先跟客人商量明白。", "三夜到了，验票收赎；无人来赎，再核票处置。"][ALL_EVENTS.find(String(event.id))]
