...
static func build(day: DayController, catalog: ContentCatalog, entry: Dictionary) -> Dictionary:
	var titles := {"acquisition": "收购成交", "pawn_loan": "活当办妥", "sale": "出售成交", "redemption": "赎当办妥", "extension": "续当办妥"}
	...
	var note: String = {"acquisition": "货已收进库存，收购款已付清。", "pawn_loan": "当票已开，在当物品留铺保管。", "sale": "货已交给买家，货款收妥。", "redemption": "赎金收妥，原物已交还当户。", "extension": "续当费收妥，原物继续留铺。"}[entry.kind]
	...
	for scenario in day.definition.trade_scenarios:
		if item.source_visit_id.ends_with("/" + scenario.slot_id):
