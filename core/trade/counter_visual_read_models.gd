...
static func enrich(model: Dictionary, day: DayController, service: CounterService, visit: CustomerVisit) -> void:
	...
	var visual := {
		...
		"social_feedback": day.state.social_enabled, "asking": FanBargainingService.asking(day.state, visit), "rounds_left": visit.trade.rounds_left,
		"intimidated": MilitaryPlaque.used(day.state, visit),
		"original_purchase_basis": maxi(1, visit.trade.opening_price - visit.trade.social_flaw_discount),
		"original_pawn_basis": maxi(1, roundi(maxi(1, visit.trade.opening_price - visit.trade.social_flaw_discount) * terms.loan_ratio)) if terms != null else 1,
