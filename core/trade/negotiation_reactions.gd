...
func record(day: DayController, visit: CustomerVisit, before: int, command: String, detail: String, result: ActionResult) -> void:
	...
	var relevant := command in ["pressure", "belittle", "concession", "offer", "pawn", "fan_pressure", "condition_pressure", "luxury_pressure", "watch_claim", "pearl_claim", "camera_claim", "porcelain_claim", "bangle_claim", "intimidate"]
	if command == "question":
