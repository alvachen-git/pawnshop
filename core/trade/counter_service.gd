...
func reason(day: DayController, command: String, visit_id: String, detail := "", amount := 0) -> String:
	...
	match command:
		...
		"pearl_claim": return PearlNegotiation.reason(day,visit,detail,amount)
...
func _execute(day: DayController, command: String, visit_id: String, detail := "", amount := 0) -> ActionResult:
	...
	match command:
		"watch_bluff": cost = 5
		"watch_claim", "pearl_claim", "porcelain_claim", "bangle_claim": cost = 5
	...
	match command:
		...
		"pearl_claim": message = PearlNegotiation.submit(day,visit,detail)
