...
func restore(data: Variant, run: RunDefinition, catalog: ContentCatalog, extended: bool) -> RunState:
	...
	for index in range(start, data.action_journal.size()):
		...
		if row.method == "counter_command" and (row.args[0] in ["fan_pressure", "condition_pressure", "watch_bluff", "watch_claim", "pearl_claim", "porcelain_claim", "bangle_claim"] or String(row.args[0]).begins_with("luxury_")) and not result.ok: return null
