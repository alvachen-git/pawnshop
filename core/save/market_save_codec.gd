...
static func restore(data: Dictionary, state: RunState, run: RunDefinition) -> String:
	...
	for row in data.sale_batches:
		...
		if row.id != "batch/%d" % (state.sale_batches.size() + 1) or row.item_ids.is_empty() or row.night < 1 or row.night > state.summaries.size() or row.start < 0 or int(row.start) % run.time_step != 0 or row.minute != row.start + 20 or row.minute >= run.night_minutes or row.minute > state.summaries[int(row.night) - 1].closed_at: return "出货批次时刻、货单或顺序无效。"
		var stamp := int(row.night) * (run.night_minutes + 1)
		if stamp + int(row.start) < last_end: return "多趟出货耗时重叠。"
		last_end = stamp + int(row.minute)
		if row.market_id != MarketService.current(run, state.run_seed, int(row.night), int(row.start)).get("id", "fixed"): return "出货没有使用出发时行情。"
...
static func sale_reason(row: Dictionary, state: RunState, run: RunDefinition, buyer: BuyerDefinition, item: ItemDefinition) -> String:
	if not run.batch_selling: return ""
...
static func validate_timing(state: RunState, run: RunDefinition, catalog: ContentCatalog) -> String:
	...
	for trip in state.sale_batches:
		...
		for visit in VarietyService.plan(run, catalog, state.run_seed):
			...
			if arrival > trip.start or arrival + int(visit.get("wait_minutes", customer.terms.wait_minutes)) <= trip.start: continue
			var ended := false
