...
func death_record(state: RunState, id: String) -> Dictionary:
	...
	return {"run_token": state.run_token, "run_id": String(state.run_definition_id), "night": state.current_night_index, "cash": state.cash, "item_id": id, "rule_id": rule.id, "cause": rule.death_cause, "item_name": item_def.display_name, "inventory_cost": financial.inventory_cost, "pawn_principal": financial.pawn_principal}
