...
func eligible(state: RunState, event: EventDefinition) -> bool:
	...
	for row in state.event_history:
		...
		if row.event_id == event.id:
			count += 1
			if state.current_night_index - int(row.night) <= event.cooldown: return false
...
func _can_finish_any(state: RunState, event: EventDefinition) -> bool:
	for choice in event.choices:
		if state.game_minutes + choice.minutes < event.window_end: return true
	return false
...
func choose(day: DayController, event_id: String, choice_id: String) -> ActionResult:
	...
	if choice == null or not eligible(day.state, event): return ActionResult.new(false, "事件或选择已不可用。")
...
func model(day: DayController, message: String) -> Dictionary:
	...
	return {"body": body, "history": history if not day.state.event_history.is_empty() else "", "buttons": buttons, "pending_id": day.state.pending_event_id}
