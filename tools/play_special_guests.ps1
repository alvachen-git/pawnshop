param(
    [ValidateSet('normal','hat','wet','bundle','bedroom','bedroom-aged')][string]$Stage = 'normal',
...
$qaRoot = Join-Path $gameRoot '.godot/qa/special-guests'
...
try {
    ...
    if ($Stage -eq 'normal') { $gameArgs += 'res://scenes/start_special_guests_late_v46.tscn' }
    else { $gameArgs += @('--script','res://tools/special_guests_preview.gd','--',"--special-stage=$Stage","--special-item=$Item","--special-wide=$($Wide.IsPresent.ToString().ToLower())") }
