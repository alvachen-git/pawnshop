param(
    [ValidateSet('normal','intro','fee','raid','pawn','supply','inquiry')][string]$Stage = 'normal',
    [string]$GodotPath = '',
    [switch]$Wide,
    [switch]$Verify
)
...
$qaRoot = Join-Path $gameRoot '.godot/qa/qingbang'
...
function Invoke-QingbangGodot([string[]]$EngineArguments, [string]$LogName) {
...
try {
    ...
    Invoke-QingbangGodot @('--headless','--path',$gameRoot,'--editor','--import','--quit') 'import.log'
    ...
    if ($Stage -eq 'normal') { $gameArgs += 'res://scenes/start_qingbang.tscn' }
    else { $gameArgs += @('--script','res://tests/qingbang_playground.gd','--',$Stage) }
    Invoke-QingbangGodot $gameArgs ('launch-' + $Stage + '.log')
    exit 0
