...
if ($manifest.content_version -ne 8) { throw 'M8A requires content_version=8.' }
...
$info = [ordered]@{
    version='M8A.1'; engine='4.6.1.stable.official.14d19694e'; target='Windows x86_64'; renderer='gl_compatibility';
    ...
    content_version=8; save_version=7; run_definition_id=$manifest.default_run_id;
    save_relative_path='user://p0/autosave_v7.json'; user_directory='%APPDATA%/GhostMarketPawnshop-M8A'; signed=$false;
...
$candidate = Join-Path $work 'Pawnshop-M8A.1-windows-x86_64.zip'
...
$zip = Join-Path $destination 'Pawnshop-M8A.1-windows-x86_64.zip'
