param(
    [ValidateSet('new', 'ready', 'willing', 'refused')][string]$Stage = 'new',
    [string]$Godot = '',
    [switch]$Wide,
    [switch]$Check
)
...
try {
    ...
    $logDir = Join-Path $projectRoot '.godot/qa/v25'
    ...
    if ($Stage -ne 'new') {
        ...
        if (-not (Test-Path -LiteralPath $fixture) -or -not (Test-Path -LiteralPath $stamp) -or (Get-Item -LiteralPath $stamp).LastWriteTimeUtc -lt $latest.LastWriteTimeUtc) {
            Write-Host 'Preparing and verifying isolated v25 previews...'
            $testLog = Join-Path $logDir 'launch-fixtures.log'
            $ErrorActionPreference = 'Continue'
            & $Godot --headless --path $projectRoot --script tests/run_mirror_endings.gd -- fixtures-only *> $testLog
            $ErrorActionPreference = 'Stop'
            ...
            Set-Content -LiteralPath $stamp -Value 'v25 verified' -Encoding Ascii
        }
    }
    $gameArgs = @('--path', $projectRoot, 'res://scenes/start_v25.tscn', '--resolution', $(if ($Wide) { '1600x900' } else { '1280x720' }))
    if ($Check) { $gameArgs += @('--headless', '--quit-after', '3') }
    if ($Stage -ne 'new') { $gameArgs += @('--', "--mirror-ending-preview=$Stage") }
    $ErrorActionPreference = 'Continue'
    Write-Host "Starting v25 ($Stage)..."
    if ($Check) {
