param(
    [ValidateSet('new', 'bedtime', 'call', 'dream', 'morning', 'second', 'ready')][string]$Stage = 'new',
    [string]$Godot = '',
    [switch]$Wide,
    [switch]$Check
)
...
try {
    ...
    $logDir = Join-Path $projectRoot '.godot/qa/v29'
    ...
    if ($Stage -ne 'new') {
        ...
        if (-not (Test-Path -LiteralPath $fixture) -or -not (Test-Path -LiteralPath $stamp) -or (Get-Item -LiteralPath $stamp).LastWriteTimeUtc -lt $latest.LastWriteTimeUtc) {
            Write-Host 'Preparing and verifying isolated v29 previews...'
            $testLog = Join-Path $logDir 'launch-fixtures.log'
            $ErrorActionPreference = 'Continue'
            & $Godot --headless --path $projectRoot --script tests/run_mirror_call.gd -- fixtures-only *> $testLog
            $ErrorActionPreference = 'Stop'
    ...
    $gameArgs = @('--path', $projectRoot, 'res://scenes/start.tscn', '--resolution', $(if ($Wide) { '1600x900' } else { '1280x720' }))
    if ($Check) { $gameArgs += @('--headless', '--quit-after', '3') }
    if ($Stage -ne 'new') { $gameArgs += @('--', "--mirror-call-preview=$Stage") }
    $ErrorActionPreference = 'Continue'
    Write-Host "Starting v29 ($Stage)..."
    if ($Check) {
