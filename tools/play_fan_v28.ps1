param(
    [ValidateSet('normal', 'upgrade', 'fan', 'informed', 'ordinary', 'urgent')][string]$Stage = 'normal',
    [string]$GodotPath = '',
    [switch]$Wide,
    [switch]$Verify
)
...
$env:APPDATA = Join-Path $gameRoot ('.godot/play-data/unified-v28-' + $Stage)
...
try {
    ...
    if ($Stage -ne 'normal') {
        $fixture = Join-Path $gameRoot '.godot/qa/fan-bargaining/fan-sound.json'
        $stamp = Join-Path $gameRoot '.godot/qa/fan-bargaining/verified.stamp'
        $latest = Get-ChildItem (Join-Path $gameRoot 'core'),(Join-Path $gameRoot 'data'),(Join-Path $gameRoot 'tests/fan_bargaining.gd') -Recurse -File |
        ...
        if (-not (Test-Path $fixture) -or -not (Test-Path $stamp) -or (Get-Item $stamp).LastWriteTimeUtc -lt $latest.LastWriteTimeUtc) {
            $ErrorActionPreference = 'Continue'
            & $GodotPath --headless --path $gameRoot --script tests/fan_bargaining.gd *> (Join-Path $logDir 'launch-fixtures.log')
            ...
            Set-Content -LiteralPath $stamp -Value 'verified v28' -Encoding Ascii
        }
    }
    $gameArgs = @('--path', $gameRoot, '--resolution', $(if ($Wide) { '1600x900' } else { '1280x720' }), 'res://scenes/start_fan_bargaining_v28.tscn')
    if ($Verify) { $gameArgs += @('--quit-after','90') }
    if ($Stage -ne 'normal') {
        $previewStage = if ($Stage -eq 'upgrade') { 'upgrade' } elseif ($Stage -eq 'fan') { 'fan-sound' } else { $Stage + '-ready' }
        $gameArgs += @('--', "--bargaining-preview=$previewStage")
    }
