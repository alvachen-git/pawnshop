...
const PHASE_LABELS := {"pre_open": "开铺前", "open": "营业中", "closed_processing": "已关门 · 店内处理", "night_resolution": "封铺 · 夜间结算", "day_summary": "日结", "run_ended": "本轮结束"}
...
func refresh() -> void:
	...
	_view.render({"description": "%s\n剩余 %d 分钟 · 查看面板不耗时\n请去鉴定、对话和交易页接待顾客。\n等待/店内行动也会让顾客继续等候。" % [PHASE_LABELS[state.phase], definition.night_minutes - int(state.game_minutes)], "message": _session.message, "has_save": _session.has_save(), "commands": commands})
	...
	if state.phase != _last_phase:
		...
		route_requested.emit(&"night" if state.phase in ["night_resolution", "day_summary", "run_ended"] else &"day")
