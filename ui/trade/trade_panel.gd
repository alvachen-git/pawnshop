...
func render(model: Dictionary) -> void:
	...
	if model.has("camera_claims"):
		...
		_bargain_toggle.show(); _bargain_toggle.disabled = false
		form.submitted.connect(func(detail: String) -> void: _emit_intent("camera_claim",_visit_id,detail))
	if model.has("porcelain_claims"):
		var form := WatchClaimForm.new(); _buttons.add_child(form); form.build(model.porcelain_claims)
		form.name = "PorcelainClaimForm"
		_bargain_toggle.show(); _bargain_toggle.disabled = false
		form.submitted.connect(func(detail: String) -> void: _emit_intent("porcelain_claim",_visit_id,detail))
	if model.has("bangle_claims"):
		var form := WatchClaimForm.new(); _buttons.add_child(form); form.build(model.bangle_claims)
		form.name = "BangleClaimForm"
		_bargain_toggle.show(); _bargain_toggle.disabled = false
		form.submitted.connect(func(detail: String) -> void: _emit_intent("bangle_claim",_visit_id,detail))
	if model.has("pearl_claims"):
		var form := WatchClaimForm.new(); _buttons.add_child(form); form.build(model.pearl_claims)
		form.name = "PearlClaimForm"
		_bargain_toggle.show(); _bargain_toggle.disabled = false
		form.submitted.connect(func(detail: String) -> void: _emit_intent("pearl_claim",_visit_id,detail))
