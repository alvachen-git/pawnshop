...
func refresh() -> void:
	...
	var needs_attention: bool = (intrusion and not _intrusion and phase == "closed_processing") or (model.held_ids != _held and not model.held_ids.is_empty()) or (model.pending_id != _pending and not model.pending_id.is_empty()) or (phase == "dead" and _phase != phase)
	_held = model.held_ids.duplicate()
	_pending = model.pending_id
	_phase = phase
	_intrusion = intrusion
	if needs_attention and _session.read_state().pending_event_id.is_empty(): route_requested.emit(&"risk")
