...
func _sync_military_reception() -> void:
	var state := _session._day.state
	var active := MilitaryIntroduction.active(state) or LuIntroduction.active(state) or SilverPolicy.active(state)
	var key := state.run_token + "/" + ("silver_introduction" if SilverPolicy.active(state) else "lu_introduction" if LuIntroduction.active(state) else MilitaryIntroduction.id(state))
